...
\end{enumerate}
So, setting such a threshold score forces auto-tagging decisions to be based on both context and text relevance. This threshold was found from observing the data as in 90\% cases when an email is actually related to a user story, their global similarity score was computed greater than or equal to 0.58. The trade-off between selecting this threshold and auto-tagging is discussed later in Chapter~\ref{ch:discussion}.  
...
Taggy was not trained based on data set\# 2, rather the trained relative wights from data set\# 1 was used to auto-tag the emails. It is clear that training based on data set\# 2 would put a very high relative weight to subject similarity as the subjects are identical to the user story titles. Using the training based on data set\# 1 reduces this bias.
...
\begin{table}
  ...
  \caption{IBM Jazz Data Set}
    \begin{tabular}{|p{2cm}|p{2cm}|r|p{1cm}|p{1.2cm}|p{1.2cm}|r|}
      \hline
